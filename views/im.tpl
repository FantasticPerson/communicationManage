...
<table class="table site_tb" width="100%" layoutH="157">
    <thead>
            	<tr>
					...
					<th width="10%" style="text-align: center">登录次数
                        <div onclick="paixu()" style="border-left-color: #f2f7fb;border-right-color: #f2f7fb" class="baixu_btn baixu_btn_name">
                        </div>
                    </th>
    ...
    <tbody class="im_manage_tbody">
   {{range .list}}
					<tr onclick="clickUser_im_list('{{.UserId}}')">
						<td style="text-align: center;cursor: pointer" onclick="showLoginView('{{.UserUsername}}')">{{.UserUsername}}<input type="hidden" value="{{.NameFlag}}" class="NameFlag" /></td>
...
<script type="text/javascript">
    ...
    function paixu(e) {
        console.log(e);
        var btn = document.getElementsByClassName('baixu_btn_name')[0];
        var tbody = document.getElementsByClassName('im_manage_tbody')[0];
        if (tbody && btn) {
            var isCountDown = false;
            var clasList = btn.classList;
            if (clasList.contains('baixu_btn')) {
                isCountDown = true;
                btn.className = 'baixu_btn_name baixu_btn_2'
            } else {
                btn.className = 'baixu_btn_name baixu_btn'
            }
            var list = {{.list}};
            console.log(list)
            ...
            for (var i = 0; i < list.length; i++) {
                var item = list[i];
                innerHtml += '<tr onclick="clickUser_im_list(' + item.uid + ')">';
                innerHtml += '<td style="text-align: center;cursor: pointer;width:109px" onclick="showLoginView('+item.uname+')">' + item.uname + '<input type="hidden" value="' + item.NameFlag + '" class="NameFlag" /></td>';
                innerHtml += '<td style="text-align: center;width:109px">' + item.urealname + '</td> ';
                innerHtml += '<td style="text-align: center;width:109px">' + item.times + '</td> ';
                innerHtml += '<td style="text-align: left;width:164px">' + item._ + '</td> ';
                innerHtml += '<td style="text-align: center;width:164px">' + item.Imsi + '</td> ';
                innerHtml += '<td style="text-align: center;width:164px">';
                if(item.Mac){
                    var tempName = item.bind ? '解绑' : '绑定';
                    innerHtml += ('<div><i class="fa fa-lg fa-mobile-phone" style="font-size: 24px;"></i>&nbsp;&nbsp;'+item.Mac+'&nbsp;&nbsp;'
                        +'<button data-show="'+item.Mac+'" class="btn btn-bind btn-success btn-xs" onclick="bind()" data-userid="'+item.uid
                        + '" value="'+item.bind+'" data-name="'+item.urealname+'">'+tempName+'</button></div>');
                }
                innerHtml += '</td> ';
                innerHtml += '<td style="text-align: center;width:164px">';
                if(item.PcMac){
                    var tempName2 = item.pcbind ? '解绑' : '绑定';
                    innerHtml += ('<div><i class="fa fa-lg fa-desktop" style="font-size: 16px;"></i>&nbsp;&nbsp;'+item.PcMac
                        +'&nbsp;&nbsp;<button data-show="'+item.PcMac+'" class="btn btn-bind btn-danger btn-xs" onclick="bind()" data-userid="'
                        +item.uid+'" value="'+item.pcbind+'" data-name="'+item.urealname+'">'+tempName2+'</button></div>');
                }
                innerHtml += '</td> ';
                innerHtml += '<td style="width: 109px;"><div> ';
                innerHtml += ('<button class="btn btn-primary btn-xs" onclick="edit('
                    +item.uid+')"><i class="glyphicon glyphicon-pencil"></i> 编辑</button> ');
                var tempName3 = item.state ? '关闭' : '打开';
                innerHtml += ('<button class="btn btn-xxx btn-xs" onclick="change()" data-userid="'+item.uid
                    +'" data-uuid="'+item.Uuid + '" value="'+item.state + '" data-name="'+item.urealname+'">'+tempName3+'</button>');
                innerHtml += '</div> </td> </tr>'
            }
